...
async function loadFromFeed() {
  ...
  try {
    ...
    for (let i = 0; i < offers.length; i++) {
      ...
      if (image) {
        try {
          cloudinaryImageUrl = await uploadToCloudinary(image, id);
        } catch (error) {
          console.error("Ошибка загрузки изображения на Cloudinary:", error);
          // Используем оригинальный URL в случае ошибки
...
// Функция для загрузки изображения на Cloudinary
async function uploadToCloudinary(imageUrl, productId) {
  return new Promise((resolve, reject) => {
    ...
    formData.append('public_id', `electrotools/${productId}`);
